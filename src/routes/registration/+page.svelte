<script lang="ts">
	...
	import { goto } from "$app/navigation";
      ...
      const setUsername = (event) => {
        username = event.target.value.toLowerCase().trim().replace(/[^a-zA-Z0-9]/g, '').replace(/\s+/g, '').slice(0,20);
  ...
  <SignedIn>
    ...
    <div class="w-screen">
      ...
      <input type="text" placeholder="Username" class:disabled={busy} class:input-error = {errorMessage!==""} class="input input-bordered input-success w-full max-w-lg" bind:value={username} on:input={setUsername}/>
      <br/>
